...
void main(in PS_IN In, out PS_OUT Out) {
	...
	Out.Out0.rgb += Weights[0].y * g_Texture.Sample(g_SamplerState, In.TexCoord + float2(0.0f,  2.0f / Parameter.y)).rgb;
	Out.Out0.rgb += Weights[0].z * g_Texture.Sample(g_SamplerState, In.TexCoord + float2(0.0f,  4.0f / Parameter.y)).rgb;
	Out.Out0.rgb += Weights[0].w * g_Texture.Sample(g_SamplerState, In.TexCoord + float2(0.0f,  6.0f / Parameter.y)).rgb;
	Out.Out0.rgb += Weights[1].x * g_Texture.Sample(g_SamplerState, In.TexCoord + float2(0.0f,  8.0f / Parameter.y)).rgb;
